...
void updateAnimation(){
	for(int i = 0; i < 8; i++){
		matrix_buffer[i] = animation_buffer[(i + anim_offset) % 12];
	}
}
...
//	  Ex10:
		  matrix_counter++;
		  if (matrix_counter >= MAX_LED_MATRIX){
		      anim_offset++;
		      if(anim_offset >= 12) anim_offset = 0;
		      updateAnimation();
			  matrix_counter = 0;
		  }
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOB, Seg1_Pin|Seg2_Pin|Seg3_Pin|ROW2_Pin
                          ...
                          |Seg7_Pin|ROW0_Pin|ROW1_Pin, GPIO_PIN_SET);
